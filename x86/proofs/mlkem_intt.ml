...
(* ZMM view: SIMD_SIMPLIFY_ABBREV_TAC gates on `read X s:int256`, so under the
   512-bit ZMM rooting it never fires and montmul terms accumulate unabbreviated
   -> blowup. This copy also matches (512)word reads. *)
...
let MLKEM_INTT_CORRECT = prove
  ...
  MAP_EVERY (fun n -> X86_STEPS_TAC MLKEM_INTT_TMC_EXEC [n] THEN
                      (SIMD_SIMPLIFY_ABBREV_TAC_ZMM[ntt_montmul; barred_x86]
