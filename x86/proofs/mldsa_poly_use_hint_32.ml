  ...
  ENSURES_WHILE_PUP_TAC `32` `pc + 0x50` `pc + 0xba`
   `\i s.
      (read RDI s = word_add a (word(32 * i)) /\
       ...
       read ZMM5 s = (word 3423913227525323174502430081042878883520180111764122672559515536195711:(512)word) /\
       read ZMM8 s = (word 27633945340263435069803077425739770516599878854789179050185066335437825:(512)word) /\
       read ZMM7 s = (word 13803492696795003664135781114125621955608915096245911876775369720726016:(512)word) /\
       read ZMM4 s = (word 218875081946729975600369013236132924539112762223623301674088649976692072704:(512)word) /\
       read ZMM6 s = (word 0:(512)word) /\
       read ZMM3 s = (word 404399200101416122972727962327899080730729934460329449514903409786895:(512)word) /\
       (!b. b < 32 ==> read(memory :> bytes256(word_add h (word(32 * b)))) s = yb b) /\
       (!b. i <= b /\ b < 32
...
let MLDSA_POLY_USE_HINT_32_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!a h x y pc stackpointer returnaddress.
  ...
  MAP_EVERY ABBREV_TAC
   [`zmm6_epilog = read ZMM6 s12`;
    `zmm7_epilog = read ZMM7 s12`;
    `zmm8_epilog = read ZMM8 s12`;
    `zmm9_epilog = read ZMM9 s12`;
    `zmm10_epilog = read ZMM10 s12`;
    `zmm11_epilog = read ZMM11 s12`] THEN
