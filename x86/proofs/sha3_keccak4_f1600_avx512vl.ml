...
(* Signature tuple for sha3_keccak4_f1600_avx512vl (a: 100 u64, in+out; rc: 24
   u64, in only), matching the shared subroutine_signatures.ml entry. *)
let sha3_keccak4_f1600_avx512vl_signature =
  ([(* args *)
     ("a", "uint64_t[100]", "false");
     ("rc", "uint64_t[24]", "true")],
   "void",
   [(* input buffers *) ("a", "100", 8); ("rc", "24", 8)],
   [(* output buffers *) ("a", "100", 8)],
   [(* temporary buffers *)]);;

let full_spec,public_vars = mk_safety_spec
    ~keep_maychanges:true
    sha3_keccak4_f1600_avx512vl_signature
    (REWRITE_RULE[SOME_FLAGS] SHA3_KECCAK4_F1600_AVX512VL_CORRECT)
    SHA3_KECCAK4_F1600_AVX512VL_EXEC;;

let SHA3_KECCAK4_F1600_AVX512VL_SAFE =
               ...
               (\s. read RIP s = returnaddress /\
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2
                           [bitstate_in,800; rc_pointer,192; bitstate_in,800; stackpointer,8]
  ...
  ==> ensures x86
         ...
         (\s. read RIP s = returnaddress /\
              ...
              (exists e2.
                    ...
                    memaccess_inbounds e2
                      [bitstate_in,800; rc_pointer,192; bitstate_in,800;
                       word_sub stackpointer (word 0xb0),0xb0 + 8]
                      [bitstate_in,800; word_sub stackpointer (word 0xb0),0xb0 + 8]))
         (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes (bitstate_in, 800);
                     memory :> bytes(word_sub stackpointer (word 0xb0), 0xb0)])`,
