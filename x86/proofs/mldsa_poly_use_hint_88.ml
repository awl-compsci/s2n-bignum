...
(* ZMM view: local SIMD_SIMPLIFY_TAC whose simdable gate also matches 512-bit
   ZMM reads, so the block body reduces the word_zx-wrapped constant reads. *)
let SIMD_SIMPLIFY_TAC_ZMM unfold_defs =
  ...
  let x86_simdable512 = can (term_match [] `read X (s:x86state):(512)word = whatever`) in
...
(* ZMM view: ABBREV_BIG_TAC only compacts int256-typed `read` values; under the
   default ZMM view the SIMD reads are (512)word, typically word_zx(<int256 YMM
   computation>), so the big block values never get abbreviated and the body
   stepping explodes.  Local variant that, for such reads, abbreviates the INT256
   inner of word_zx (not the whole (512)word value): this keeps the abbreviation
   chain int256 (so the stored bytes256 value collapses word_zx(word_zx _)=_ back
   to the plain int256 atom and the downstream int256-shaped store discharge is
   unchanged).  No-op under the YMM view. *)
let ABBREV_BIG_TAC_ZMM : tactic = fun (asl,w) ->
  MAP_EVERY (fun (_,th) ->
      let v = rand(concl th) in
      let tgt =
        if type_of(lhs(concl th))=`:(512)word` && is_comb v
    ...
    (filter (fun (_,th) -> let c=concl th in is_eq c &&
       (try is_comb(lhs c) && fst(dest_const(fst(strip_comb(lhs c))))="read"
            && (type_of(lhs c)=`:int256` || type_of(lhs c)=`:(512)word`)
...
let MLDSA_POLY_USE_HINT_88_BLOCK_CORRECT = prove
 (`!a h xb yb pc.
  ...
  (* Loop invariant.  The VPBROADCASTD constant registers are pinned at their
     ...
     store/wrapper discharges match on the numeral shape.  The constants are:
       ZMM8 = 11275   ZMM4 = 127   ZMM7 = 128
       ZMM6 = 43   ZMM3 = 8285184   ZMM5 = 0. *)
  ENSURES_WHILE_PUP_TAC `32` `pc + 0x52` `pc + 0xd7`
   `\i s.
      (read RDI s = word_add a (word(32 * i)) /\
       ...
       read ZMM8 s = (word 303973398742897785767833851683137475682598667402680969552035729689816075:(512)word) /\
       read ZMM4 s = (word 3423913227525323174502430081042878883520180111764122672559515536195711:(512)word) /\
       read ZMM7 s = (word 3450873174198750916033945278531405488902228774061477969193842430181504:(512)word) /\
       read ZMM6 s = (word 1159277706957392885855153492006644031428092478786277755276056441389099:(512)word) /\
       read ZMM3 s = (word 223368118819536749293045209988780814485663464087451345989979032820788390912:(512)word) /\
       read ZMM5 s = (word 0:(512)word) /\
       (!b. b < 32 ==> read(memory :> bytes256(word_add h (word(32 * b)))) s = yb b) /\
       (!b. i <= b /\ b < 32
