...
(* ZMM view: SIMD_SIMPLIFY_ABBREV_TAC gates on `read X s:int256`, so under the
   512-bit ZMM rooting (register reads are `read ZMMn s:(512)word`) it fires on
   nothing and the montmul word-terms accumulate unabbreviated -> blowup. This is
   a copy whose gate also matches the (512)word reads, so the per-step simplify +
   abbreviate happens as in the YMM view. *)
...
let MLKEM_NTT_CORRECT = prove
  ...
  MAP_EVERY (fun n -> X86_STEPS_TAC MLKEM_NTT_TMC_EXEC [n] THEN
                      (SIMD_SIMPLIFY_ABBREV_TAC_ZMM [ntt_montmul] [ntt_montmul_add; ntt_montmul_sub]))
        (1--587) THEN
