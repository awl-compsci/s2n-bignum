...
(* ZMM view: local SIMD_SIMPLIFY_TAC whose simdable gate also matches 512-bit
   ...
   appear as full-ZMM writes.  No-op under the YMM view. *)
...
let MLDSA_DECOMPOSE_88_CORRECT = prove(
 `!a1 a (x:num->int32) pc.
  ...
  MAP_EVERY (fun n ->
    ...
    SIMD_SIMPLIFY_TAC_ZMM[decompose88_a1; decompose88_a0]) (1--399) THEN
  ...
  (* ZMM view: collapse the read-ZMM word_zx round-trips (widen-to-512 then
     ...
     the DECOMPOSE88_* lemma matches below succeed.  No-op under the YMM view. *)
  ...
  (* ZMM view: now that the word_zx wrapper is gone, re-run the WORD_NOT_JOIN
     ...
     first pass above already handled this).  No-op under the YMM view. *)
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_NOT_JOIN_256; WORD_NOT_JOIN_128; WORD_NOT_JOIN_64]) THEN
...
let MLDSA_DECOMPOSE_88_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!a1 a (x:num->int32) pc stackpointer returnaddress.
  ...
  MAP_EVERY ABBREV_TAC
   [`zmm10_epilog = read ZMM10 s11`;
    `zmm11_epilog = read ZMM11 s11`;
    `zmm12_epilog = read ZMM12 s11`;
    `zmm13_epilog = read ZMM13 s11`;
    `zmm14_epilog = read ZMM14 s11`] THEN
